...
// 현재 처리 중인 함수의 반환 타입을 저장 (Return 문 검사용)
...
static void checkNode(TreeNode* t)
{
    ...
    case ExpK:
        ...
        case CallK:
            ...
            else if (l->node->nodekind != DeclK || l->node->kind.decl != FunK) {
    ...
    case StmtK:
        ...
        case ReturnK:
        {
            ExpType returnType = (t->child[0] == NULL) ? Void : t->child[0]->type;
...
// 최종 의미 분석 메인 함수 (Single Pass)
void analyze(TreeNode* syntaxTree)
{
    // 심볼 테이블은 main.c에서 초기화됨
    currentScope = 0;
    location[0] = 0;

    if (TraceParse) fprintf(listing, "\nBuilding Symbol Table & Checking Types...\n");
